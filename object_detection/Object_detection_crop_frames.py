...
while ret:
    ...
    for i in range(len(score_list)):
        if score_list[i] < 0.9:
            ...
            #---print(str(boxes[i])) # ymin, xmin, ymax, xmax
            ...
            #---box = [int(xmin * width), int(xmax * width), int(ymin * height), int(ymax * height)]
            #---print(str(box) + "\n") # xmin, xmax, ymin, ymax desnormalizado
            
            ## TROCAR YMIM YMAX XMIM XMAX POR LEFT RIGHT TOP BOTTOM
            
            crop_height = int((ymax - ymin)*1.6)
            crop_width = crop_height
            
            xini = int((xmin + 3*(xmax - xmin)/8) - crop_width/2) # centro em x - largura/2 para centralizar a largura
            yini = int((ymin + (ymax - ymin)/2) - crop_height/2)
            
            if((xini > 0 and yini > 0) and ((xini+crop_width) < width and (yini+crop_height) < height)):
                if class_list[i] < 4: # somente discos
                    crop = frame_original[yini:(yini+crop_height), xini:(xini+crop_width)]
                    name_path = "./crop_detection/classe_" + str(class_list[i]) + "_D" + str(disc_count) + "_" + str(frame_count) + ".jpg"
                    cv2.imwrite(name_path, crop)
